...
class Advertisement(dbus.service.Object): # 광고 패킷의 내용 담는 클래스
    ...
    def update_service_data(self, packet):
        try:
            if packet == self._last_payload:
                print(f"No payload change")
                return True
            self.service_data[dbus.String(bluetooth_constants.ENVIRONMENT_SENSING_UUID)] = dbus.ByteArray(packet)
            self._last_payload = packet

            changed_props = dbus.Dictionary({
                'ServiceData': dbus.Dictionary(self.service_data, signature='sv')
            }, signature='sv')
            self.PropertiesChanged('org.bluez.LEAdvertisement1', changed_props, dbus.Array([], signature='s'))
            return True
