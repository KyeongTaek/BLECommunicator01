...
class Advertisement(dbus.service.Object): # 광고 패킷의 내용 담는 클래스
    ...
    # 데이터가 바뀌면 dbus를 통해 데이터가 바뀌었다는 내용의 PropertiesChanged 신호 보내도록 함.
    # 신호를 BlueZ가 감지하면, 속성을 다시 조회해서 실질적으로 광고를 갱신함
    @dbus.service.signal(dbus.PROPERTIES_IFACE, signature='sa{sv}as') # 형태: 인터페이스 이름(string), 변경된 속성들(dict), 무효화된 속성들(array of strings)
    def PropertiesChanged(self, interface, changed, invalidated):
        pass

    def update_service_data(self, packet): # 센서 패킷 받아 데이터 갱신하는 메서드
        try:
            if packet == self._last_payload: # 들어온 패킷이 이전 패킷과 동일하면
                print(f"No payload change") # 변경 없음
                return True
            # 들어온 패킷이 이전 패킷과 다른 경우
            self.service_data[dbus.String(bluetooth_constants.ENVIRONMENT_SENSING_UUID)] = dbus.ByteArray(packet) # 데이터 갱신
            self._last_payload = packet # 이전 패킷으로 저장

            # 바뀐 부분에 대한 정보를 신호에 담음.
            changed_props = dbus.Dictionary({ # BlueZ가 이해할 수 있는 타입으로 규격화
                'ServiceData': dbus.Dictionary(self.service_data, signature='sv')
            }, signature='sv')
            self.PropertiesChanged('org.bluez.LEAdvertisement1', changed_props, dbus.Array([], signature='s')) # 버스를 통해 데이터 갱신 신호 보냄(LEAdvertisement1 중 changed_props 부분이 바뀌었음을 명시)
            return True
...
if __name__ == '__main__': # 실행된 파일이 server_advertising.py인 경우
    ...
    import data_handler # 센서 패킷 가져오는 모듈
    GLib.timeout_add(1000, data_handler.run_update, adv) # 1초(1000ms)마다 run_update 함수 호출(=1초마다 센서 패킷 가져옴)
